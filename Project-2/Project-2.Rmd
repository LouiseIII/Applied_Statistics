...
To this end, we will proceed in 3 parts. In the first one, we will explore and treat the data to understand some client's behaviors and obtain some intuitions. Then in the second part, we will try to find an appropriate model. This will lead us to the last part, the interpretation and discussion of the model. 
...
In this figure, we can already notice some habits of potential customers: they spend much more time on product pages than on administration and information ones. This may seem logical since they consult product pages much more than the others. 

We can also observe that the number of "other visitor" is underrepresented in the data  as well as the number of value around a special day. Or some month where we even have no observation like january or april. 
...
Moreover, we can observe something similar with the `special_day` variable, as shown in Figure 3. Indeed, we can even see that when we are very close to a special day, every person, who arrives on the website, buy something.
...
Moreover, in the same idea the more time the customer spends looking at product pages, the more likely they are to buy. Or the more time he wastes on administrative pages, the less likely he is to purchase something. 
